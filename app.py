...
def render_sidebar(mcp_tools: Optional[Dict[str, List[MCPTool]]] = None, llm_client = None):
    ...
    with st.sidebar:
        ...
        with id_about_tab:
            st.info(
                """
                Chatbot Internal Sibernetik adalah asisten virtual berbasis Large Language Model (LLM) yang terintegrasi 
                dengan sistem internal perusahaan. Dengan dukungan Model Context Protocol, chatbot ini memahami konteks 
                pengguna, histori percakapan, dan role-based access, sehingga dapat memberikan jawaban yang tepat dan aman. 
                Chatbot mampu mengeksekusi tugas seperti mengambil data, membuat tiket, dan menampilkan laporan melalui integrasi API. 
                Selain itu, chatbot dapat menjawab pertanyaan seputar SOP, kebijakan, atau status proyek dengan percakapan yang berkelanjutan. 
                Solusi ini mendukung efisiensi kerja dan kolaborasi tim di lingkungan Sibernetik secara otomatis dan cerdas.
                """
            )
        with en_about_tab:
            st.info(
                """
                Sibernetik’s Internal Chatbot is a virtual assistant powered by a Large Language Model (LLM), 
                integrated with the company’s internal systems. Using a Model Context Protocol, 
                it understands user roles, conversation history, and context to deliver accurate and secure responses. 
                The chatbot can execute tasks such as retrieving data, creating support tickets, and generating reports via API integrations. 
                It also answers questions related to SOPs, policies, or project status through ongoing, context-aware conversations. 
                This intelligent assistant enhances daily operations, boosts team collaboration, and streamlines workflows across the Sibernetik environment with smart automation.
                """
            )
